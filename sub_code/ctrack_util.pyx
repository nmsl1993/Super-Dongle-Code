...
cdef double PHASE_THRESH = .0002
cdef double MAG_THRESH = 2.0e10
cdef double PING_COOLDOWN = 2000
...
cdef render_plots(samples, phase, magnitude, variance):
    ...
    do_plot(4, variance, "Phase Variance", blocking=True)#, PHASE_THRESH, True)
    ...
    for i in range(length):
        ...
        if phase0.variance() < PHASE_THRESH and dtft_0_0.mag_sq() > MAG_THRESH:
            ...
            if i > lastping + PING_COOLDOWN:
                lastping = i
                heading = cmath.atan2(phase0.average(), phase1.average())*180/PI
            ...
            hlstart = -1
        phases.append(cmath.atan2(phase0.average(), phase1.average())*180/PI)
